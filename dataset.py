...
class ImageClassificationDataset(torch.utils.data.Dataset):
    ...
    def save_entry(self, image, category):
        ...
        if not os.path.exists(category_directory):
            try:
                os.makedirs(category_directory) #makedirs es para windows
            except OSError:
                print ("Creation of the directory %s failed" % category_directory)
            else:
                print ("Successfully created the directory %s " % category_directory)
